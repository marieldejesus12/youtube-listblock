...
# the Pi-hole log, generate a local block list for immediate effect and
...
function move_rename() {
  ...
  if [[ "$0" != "/usr/bin/pihole_collect.sh"  ]]; then
    echov "Moving script to /usr/bin"
    mv $0 /usr/bin/pihole_collect.sh
    chmod +x /usr/bin/pihole_collect.sh
    SCRIPT="pihole_collect.sh"
  fi
  #Configure cron
...
function update() {
  ...
  FILETMP=/tmp/pihole.txt
  LOCALLIST=/var/www/html/youtube.txt
  ...
  grep r*.googlevideo.com $LOCALLISTLOG \
  | awk '{print $6}' \
